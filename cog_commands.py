...
!shuffle - Plays shuffled queue
!remove <song_number> - Removes the song from queue
"""
    ...
    @commands.command(name='now', aliases=['source'])
    async def now(self, ctx: commands.Context):
        await ctx.send(embed=self.music_queue.current.create_embed())
    ...
    async def shuffle(self, ctx: commands.Context):
        if len(self.music_queue) == 0:
            return await ctx.send('Queue is empty now')

        self.music_queue.shuffle()
        await ctx.send('Now playing suffled queue!!')
    ...
    @commands.command(name='previous')
    async def previous(self, ctx: commands.Context, index: int):
        video_source = self.hidden_music_queue[index][0]['source']
        if len(self.music_queue) == 0:
            return await ctx.send('Empty queue.')
